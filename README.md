...
| `:pxf-runtime` | `org.protowire.pxf` | The descriptor-free half of `:pxf` (`Ast`, `Lexer`, `Parser`, `Format`, `Result`, `Limits`) plus the lite-tier metadata contract (`PxfMeta`, `PxfRegistry`, `PxfEnum`, `TimeFormats`). No protobuf-java dependency; `:pxf` re-exports it. |
| `:sbe-runtime` | `org.protowire.sbe`, `org.protowire.sbe.runtime` | `View`, `ViewSchema` and the template classes, plus the descriptor-free SBE wire codec used by generated lite code. `:sbe` re-exports it. |
| `:pxf-android` | `org.protowire.pxf.android` | PXF for protobuf-javalite: `LiteWireWriter` / `LiteWireReader` / `LitePxf`, driven by `PxfMeta` classes generated by protowire's `protoc-gen-pxf-java-meta`. |
| `:pb-android`, `:sbe-android`, `:envelope-android` | (lite tier) | javalite-facing entry points: protobuf-javalite re-exported, the SBE runtime, and the `envelope/v1` messages compiled for javalite with their `PxfMeta`. |
| `:dump-envelope`, `:bench-pxf`, `:bench-sbe`, `:check-decode`, `:*-android` harnesses | (test harnesses) | Per-port binaries used by the cross-port runner scripts in the spec repo (`cross_envelope_check.sh`, `cross_pxf_bench.sh`, `cross_sbe_bench.sh`, `cross_security_check.sh`). Not published. |
