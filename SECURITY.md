...
This policy covers `protowire-java` — the Java port of the `protowire`
stack. Cross-port issues (the same input affecting multiple language
ports) are also accepted here and routed to the upstream project; you
...
- Schema-validation bypasses that let invalid messages reach
  application code.
- Reflection-driven gadgets through `protobuf-java`'s `DynamicMessage`
  surface — please report even if they require a malicious schema.
...
- Issues in `protobuf-java` itself — file those upstream at
...
released to Maven Central under `org.protowire:protowire-pxf` (and
sibling artifacts).
